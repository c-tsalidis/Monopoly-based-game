...
void setup() // only runs once at the start of the program
{
  size(1000, 1000); // size of the program's window
...
void checkTurn() // checks whose turn it is
{
...
void checkCounter(Player player) // checks the counter of the player
{
...
void checkPropertyInfo() // checks who owns the property...
{
...
void buyMenu() // buying properties menu
{
...
void rollDice() // it rolls the dice --> gives a random number between 1 and 6
{
  dice = int(random(1, 7)); // up to 7 because it's not included
...
void checkMoney() // check money to see if anyone is bankrupt
{
...
boolean isInHitbox(Property property) // if the mouse is inside of the property's hitbox
{
...
boolean isInHitbox(Button button, int currentState) // if the mouse is inside of the button's hitbox and the game is in a specific state
{
